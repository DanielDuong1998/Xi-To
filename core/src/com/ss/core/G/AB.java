...
public class AB {//Action builder
  ...
  public AB s(float x, float y) {
    props.put("scale", new Vector2(x,y));
    return this;
  }
  ...
  private static Array<ParallelAction> buildAppearance() {
    ...
    Vector2 ps = (Vector2) props.get("scale");
    ...
    if (ps != null) {
      ScaleToAction sa = Actions.scaleTo(ps.x,ps.y);
      ScaleToAction rsa = Actions.scaleTo(target.getScaleX(), target.getScaleY());
      pa.addAction(sa);rpa.addAction(rsa);
    }
